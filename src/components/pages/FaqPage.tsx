...
const PRODUCT: readonly QA[] = [
  { q: 'What is Dev X-Ray?', a: 'A browser-first developer toolkit: 24 tools for formatting, decoding, converting and inspecting the kinds of data you deal with daily — JSON, GraphQL, JWTs, YAML, SQL, cron expressions, CSV and more — that run in your browser rather than on a server.' },
  ...
  { q: 'Why 24 tools?', a: 'There was no target number. Tools accumulated as they were needed. The count is a consequence, not a design goal — and it is deliberately not the headline claim, because the number of tools is not what makes it different.' },
  ...
  { q: 'Does it work offline?', a: 'Yes, after the first visit. The service worker precaches the whole application — 78 files, about 5.9 MB — including the editor. Verified by disabling the network and confirming that the app boots and that tools never opened before still load and compute.' },
...
const PRIVACY: readonly QA[] = [
  ...
  { q: 'What happens when I drop a file?', a: 'The browser reads it locally and routes it to the tool that handles that extension — text files as text, images as raw files handed straight to the Image tool. It is not uploaded — there is no upload endpoint. Files over 25 MB are refused, and only the first file of a multi-file drop is used.' },
  { q: 'Does the Image tool strip metadata?', a: 'Yes. Re-encoding an image through the Image tool strips EXIF metadata — including GPS location — so the downloaded file carries no hidden data about where or when it was taken. It also drops the ICC color profile, which can shift colors slightly on wide-gamut source photos. This is a side effect of re-encoding through the browser’s Canvas API, not an optional setting — it always happens and cannot be skipped.' },
...
const SECURITY: readonly QA[] = [
  ...
  { q: 'Why did a tool refuse my input?', a: 'Because it exceeded that tool\u2019s resource ceiling. Everything runs on the main thread of this tab, so an unbounded operation freezes the window you are working in rather than failing somewhere else. Each tool has its own limit measured in UTF-8 bytes, set from measured cost \u2014 JSON and CSV sit around 10 MB, YAML at 1 MB because its parser slows with structural complexity rather than size, and Markdown at 512 KB because its cost is the DOM it produces. The error names the size and the ceiling. Nothing is silently truncated.' },
  { q: 'A huge CSV only shows some rows \u2014 is the rest lost?', a: 'No. The whole file is parsed; only the table rendering is bounded. A 12 MB CSV parses in about 170 ms but would produce hundreds of thousands of table rows, which is what actually kills a tab. The footer says how many rows are shown, and Copy JSON and Copy TSV include every parsed row.' },
  { q: 'What happens if my browser storage is full?', a: 'The session stops being saved and you are told once. Preferences, history and Mapper state live in localStorage; if the origin is over quota the write is dropped rather than thrown, and every tool keeps working in memory for the rest of the session.' },
...
const TECHNICAL: readonly QA[] = [
  ...
  { q: 'Why is Monaco bundled instead of loaded from a CDN?', a: 'Two reasons. A CDN request on every page load would contradict the promise that the application makes no third-party requests, and it would make the offline promise false — the shell would cache but the editor would not load.' },
  ...
  { q: 'What is Zustand used for?', a: 'Four stores: preferences (persisted), history (persisted), Mapper (persisted, under its own key) and ephemeral UI state. They are consumed directly rather than through a facade so a component only re-renders for the slice it actually reads.' },
