...
const PRODUCT: readonly QA[] = [
  ...
  { q: 'Does it work offline?', a: 'Yes, after the first visit. The service worker precaches the whole application — 66 files, about 5.3 MB — including the editor. Verified by disabling the network and confirming that the app boots and that tools never opened before still load and compute.' },
...
const SECURITY: readonly QA[] = [
  ...
  { q: 'Is the source code available?', a: 'Yes, the repository is public and readable. Note that no licence file is currently declared, which means it is source-available rather than formally open source — worth knowing if you intend to host or reuse it.' },
  { q: 'Are dependencies audited?', a: 'Dependency versions are pinned by a lockfile and every dependency is bundled to run locally rather than as a hosted service. There is no formal third-party audit or published SBOM, and supply-chain compromise remains a real risk for any JavaScript project.' },
];
